...
@Component({
  selector: 'app-mini-book-view',
  templateUrl: './mini-book-view.page.html',
  styleUrls: ['./mini-book-view.page.scss'],
  standalone: true,
  imports: [
    IonRefresher,
    IonRefresherContent,
...
export class MiniBookViewPage implements OnInit {
  miniBook: any;
  purchase: any;
  ...
  async getMiniBook(_id: string) {
    this.miniBook = {};
    this.isite
      .api({
        url: '/api/miniBooks/viewToStudent',
        body: {
          _id: _id,
        },
      })
  ...
  async buyMiniBook() {
    ...
    this.isite
      .api({
        url: '/api/miniBooks/buyCode',
        body: {
          purchase: this.purchase,
          miniBookId: this.miniBook.id,
          miniBookPrice: this.miniBook.price,
        },
      })
      .subscribe((res: any) => {
        if (res.done) {
